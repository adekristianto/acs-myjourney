...
function WisdomScreen({ onBack, onComplete }) {
  ...
  const handleSubmit = () => {
    if (!note.trim()) {
      alert(t('journal.required') || '⚠️ Mohon isi pembelajaran hari ini')
      return
    }
    onComplete()
    alert('✅ ' + t('dimensions.wisdom') + ' — ' + (t('journal.saved') || 'Jurnal disimpan!'))
    onBack()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0D0D0D]">
      <div className="flex items-center p-4 bg-white dark:bg-[#151515] border-b border-gray-200 dark:border-gray-700">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t('journal.wisdom')}</h1>
      </div>
      <div className="p-4 space-y-4">
        <div>
          <label className="block font-medium mb-1 text-gray-900 dark:text-white">
            📝 {t('journal.wisdomPrompt') || 'Apa yang kamu pelajari hari ini?'}
          </label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder={t('journal.wisdomPlaceholder') || 'Tuliskan pembelajaran baru...'}
            rows="5"
