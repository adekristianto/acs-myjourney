...
function SocialScreen({ onBack, onComplete }) {
  ...
  const handleSubmit = () => {
    if (!note.trim()) {
      alert('⚠️ Mohon isi interaksi sosial hari ini')
      return
    }
    onComplete()
    alert('✅ Sosial — Jurnal disimpan!')
    onBack()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0D0D0D]">
      ...
      <div className="flex items-center p-4 bg-white dark:bg-[#151515] border-b border-gray-200 dark:border-gray-700">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Sosial</h1>
      </div>
      ...
      <div className="p-4 space-y-4">
        <div>
          <label className="block font-medium mb-1 text-gray-900 dark:text-white">
            🤝 Bagaimana interaksi sosialmu hari ini?
          </label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder="Contoh: Bertemu teman, membantu orang lain, quality time dengan keluarga..."
            rows="5"
