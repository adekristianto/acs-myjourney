...
function WorkerAttendanceScreen({ onBack }) {
  ...
  const handleSave = () => {
    ...
    alert(`✅ Absen Meeting Pekerja berhasil!\nHadir: ${presentCount} dari ${attendance.length} orang`)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0D0D0D]">
      ...
      <div className="flex items-center p-4 bg-white dark:bg-[#151515] border-b border-gray-200 dark:border-gray-700">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Meeting Pekerja</h1>
      </div>
      ...
      <div className="p-4">
        <div className="p-3 rounded-lg bg-orange-50 dark:bg-orange-900/20 border border-[#FF8A00]">
          <p className="text-sm text-[#FF8A00]">
            📋 Checklist kehadiran staff & pelayan
          </p>
        </div>
      </div>
      ...
      <div className="p-4">
        <button
          onClick={handleSave}
          className="w-full py-3 rounded-lg font-medium transition-colors bg-black dark:bg-[#FF8A00] text-white dark:text-black hover:bg-gray-800 dark:hover:bg-[#E67A00]"
        >
          💾 Simpan Absen
        </button>
      </div>
